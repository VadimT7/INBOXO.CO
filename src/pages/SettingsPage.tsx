        ...
        >
          <Tabs defaultValue="account" className="space-y-6">
            <TabsList className="grid w-full grid-cols-7">
              ...
              <TabsTrigger value="leads">Lead Rules</TabsTrigger>
              ...
              <Card>
                ...
                <CardContent className="space-y-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <Label htmlFor="autoClassify">Enable Auto Classification</Label>
                      <p className="text-sm text-slate-600">Automatically classify leads based on keywords and patterns</p>
                    </div>
                    <Switch
                      id="autoClassify"
                      checked={settings.leadClassification.autoClassify}
                      onCheckedChange={(checked) => updateSettings('leadClassification.autoClassify', checked)}
                    />
                  </div>

                  <div>
                    <Label htmlFor="urgencyThreshold">Urgency Threshold</Label>
                    <div className="mt-2">
                      <Progress value={settings.leadClassification.urgencyThreshold} className="w-full" />
                      <div className="flex justify-between text-sm text-slate-600 mt-1">
                        <span>Low</span>
                        <span>{settings.leadClassification.urgencyThreshold}%</span>
                        <span>High</span>
                      </div>
                      <Input
                        type="range"
                        min="0"
                        max="100"
                        value={settings.leadClassification.urgencyThreshold}
                        onChange={(e) => updateSettings('leadClassification.urgencyThreshold', parseInt(e.target.value))}
                        className="mt-2"
                      />
                    </div>
                  </div>
                  ...
                  {(['hot', 'warm', 'cold'] as const).map((type) => (
                    <div key={type}>
                      <Label className="capitalize">{type} Lead Keywords</Label>
                      <p className="text-sm text-slate-600 mb-2">
                        Keywords that automatically classify leads as {type}
                      </p>
                      <div className="flex flex-wrap gap-2 mb-2">
                        {settings.leadClassification[`${type}Keywords`].map((keyword) => (
                          <Badge 
                            key={keyword} 
                            variant="secondary"
                      ...
                      <div className="flex gap-2">
                        <Input
                          placeholder={`Add ${type} keyword...`}
                          onKeyPress={(e) => {
                            if (e.key === 'Enter') {
                              addKeyword(type, e.currentTarget.value);
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between p-4 border rounded-lg">
                    <div className="flex items-center space-x-3">
                      <div className={`w-3 h-3 rounded-full ${settings.gmailIntegration.isConnected ? 'bg-green-500' : 'bg-red-500'}`}></div>
                      <div>
                        <p className="font-medium">Gmail Account</p>
                        <p className="text-sm text-slate-600">
                          {settings.gmailIntegration.isConnected ? 'Connected' : 'Not connected'}
                        </p>
                      </div>
                    </div>
                    <Button
                      variant={settings.gmailIntegration.isConnected ? "destructive" : "default"}
                      onClick={settings.gmailIntegration.isConnected ? disconnectGmail : connectGmail}
                    >
                      {settings.gmailIntegration.isConnected ? 'Disconnect' : 'Connect Gmail'}
                    </Button>
                  </div>

                  {settings.gmailIntegration.isConnected && (
                    <>
                      <div>
                        <Label htmlFor="syncFrequency">Sync Frequency</Label>
                        <Select 
                          value={settings.gmailIntegration.syncFrequency} 
                          onValueChange={(value) => updateSettings('gmailIntegration.syncFrequency', value)}
                        >
                          <SelectTrigger className="mt-1">
                            <SelectValue />
                          </SelectTrigger>
                      ...
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          <div>
                            <Label htmlFor="autoRespond">Auto-Respond</Label>
                            <p className="text-sm text-slate-600">Send automatic responses to new leads</p>
                          </div>
                          <Switch
                            id="autoRespond"
                            checked={settings.gmailIntegration.autoRespond}
                            onCheckedChange={(checked) => updateSettings('gmailIntegration.autoRespond', checked)}
                          />
                        </div>
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-sm text-slate-600">Use smart response templates</p>
                          </div>
                          <Switch
                            id="responseTemplates"
                            checked={settings.gmailIntegration.responseTemplates}
                            onCheckedChange={(checked) => updateSettings('gmailIntegration.responseTemplates', checked)}
                          />
                        </div>
                      </div>
                    </>
